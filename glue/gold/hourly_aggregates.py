...
TARGET_PATH = "s3://coin-project-newone/gold/hourly_aggregates/"
...
clean_df = (
    df.select(
        ...
        F.col("ingest_hour").cast("int").alias("ingest_hour"),
    ...
    .filter(F.col("ingest_hour").isNotNull())
)

hourly_agg_df = (
    clean_df.groupBy("id", "symbol", "ingest_date", "ingest_hour")
    .agg(
...
print("Hourly aggregates full refresh completed successfully.")
